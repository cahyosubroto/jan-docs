...
Update the `openai.json` file in `~/jan/engines` to include the LM Studio server's full URL.
...
3. Inside, craft a `model.json` file with these directives:
   - Set `format` to `api`.
...
2. Click "Use" to utilize the model.
